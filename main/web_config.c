...
static esp_err_t config_page_handler(httpd_req_t *req)
{
    ...
    httpd_resp_sendstr_chunk(req,
        ...
        "  h += '<label><strong>Sensor Name *:</strong></label><input type=\"text\" name=\"sensor_' + sensorCount + '_name\" placeholder=\"e.g., Tank 1 Water Quality\" style=\"width:300px;border:2px solid #dc3545\" required><br>';"
        "  h += '<label><strong>Unit ID *:</strong></label><input type=\"text\" name=\"sensor_' + sensorCount + '_unit_id\" placeholder=\"e.g., TFG2235Q\" style=\"width:300px;border:2px solid #dc3545\" required><br>';"
        "  h += '<label><strong>Description:</strong></label><input type=\"text\" name=\"sensor_' + sensorCount + '_description\" placeholder=\"e.g., Main tank water quality monitoring\" style=\"width:300px\"><br>';"
    ...
    httpd_resp_sendstr_chunk(req,
        ...
        "}"
        "let formHtml = '';"
        "formHtml += '<style>.sensor-card label { display:inline-block; width:160px; font-weight:600; vertical-align:top; } .sensor-form-grid { display:grid; grid-template-columns:160px 1fr; gap:10px; align-items:start; margin-bottom:10px; }</style>';"
        "formHtml += '<div class=\"sensor-form-grid\">';"
        "formHtml += '<label><strong>Name *:</strong></label><input type=\"text\" name=\"sensor_' + sensorId + '_name\" placeholder=\"e.g., anil\" style=\"width:200px;padding:8px;border:2px solid #dc3545;border-radius:4px\" required>';"
        "formHtml += '<label><strong>Unit ID *:</strong></label><input type=\"text\" name=\"sensor_' + sensorId + '_unit_id\" placeholder=\"e.g., fg1234\" style=\"width:200px;padding:8px;border:2px solid #dc3545;border-radius:4px\" required>';"
        "formHtml += '<label>Slave ID:</label><input type=\"number\" name=\"sensor_' + sensorId + '_slave_id\" value=\"1\" min=\"1\" max=\"247\" style=\"width:80px;padding:8px;border:1px solid #ddd;border-radius:4px\">';"
        "formHtml += '<label>Register Address:</label><input type=\"number\" name=\"sensor_' + sensorId + '_register_address\" value=\"0\" min=\"0\" max=\"65535\" style=\"width:120px;padding:8px;border:1px solid #ddd;border-radius:4px\">';"
        "formHtml += '<label>Quantity (registers):</label><input type=\"number\" name=\"sensor_' + sensorId + '_quantity\" value=\"1\" min=\"1\" max=\"125\" style=\"width:80px;padding:8px;border:1px solid #ddd;border-radius:4px\">';"
        "formHtml += '<label>Register Type:</label><select name=\"sensor_' + sensorId + '_register_type\" style=\"width:250px;padding:8px;border:1px solid #ddd;border-radius:4px\">';"
        ...
        "if (sensorType !== 'ZEST') {"
        "formHtml += '<div class=\"sensor-form-grid\">';"
        "formHtml += '<label>Data Type:</label><select name=\"sensor_' + sensorId + '_data_type\" style=\"width:280px;padding:8px;border:1px solid #ddd;border-radius:4px\" onchange=\"showDataTypeInfo(this, ' + sensorId + ')\">';"
        ...
        "}"
        "formHtml += '<div class=\"sensor-form-grid\">';"
        "formHtml += '<label>Baud Rate:</label><select name=\"sensor_' + sensorId + '_baud_rate\" style=\"width:150px;padding:8px;border:1px solid #ddd;border-radius:4px\">';"
        ...
        "formHtml += '<label>Parity:</label><select name=\"sensor_' + sensorId + '_parity\" style=\"width:120px;padding:8px;border:1px solid #ddd;border-radius:4px\">';"
    ...
    httpd_resp_sendstr_chunk(req,
        "<div style='display:grid;grid-template-columns:180px 1fr;gap:12px;align-items:center;margin:15px 0'>"
        "<label style='font-weight:600'>Slave ID:</label>"
        "<input type='number' id='write_single_slave' min='0' max='247' value='1' style='width:100px;padding:8px;border:1px solid #ddd;border-radius:4px'>"
        "<label style='font-weight:600'>Register Address:</label>"
        "<input type='number' id='write_single_addr' min='0' max='65535' value='0' style='width:150px;padding:8px;border:1px solid #ddd;border-radius:4px'>"
        "<label style='font-weight:600'>Value (decimal):</label>"
        "<input type='number' id='write_single_value' min='0' max='65535' value='0' style='width:150px;padding:8px;border:1px solid #ddd;border-radius:4px'>"
        "</div>"
        "<button onclick='writeSingleRegister()' style='background:#007bff;color:white;padding:10px 16px;border:none;border-radius:4px;font-weight:600;cursor:pointer'>Write Single Register</button>"
        "<div id='write_single_result' style='margin-top:12px;padding:10px;background:#f8f9fa;border-radius:4px;display:none'></div>"
        "</div>"
    );
    ...
    httpd_resp_sendstr_chunk(req,
        "<div class='sensor-card' style='padding:var(--space-lg)'>"
        "<h3 style='margin-top:0;margin-bottom:12px;color:var(--color-primary)'>Write Multiple Registers (Function Code 16)</h3>"
        "<p style='margin-bottom:15px;color:#666;font-size:14px'>Write multiple consecutive values to Modbus holding registers for bulk configuration.</p>"
        "<div style='display:grid;grid-template-columns:180px 1fr;gap:12px;align-items:start;margin-bottom:15px'>"
        "<label style='font-weight:600;padding-top:8px'>Slave ID:</label>"
        "<input type='number' id='write_multi_slave' min='0' max='247' value='1' style='width:100px;padding:8px;border:1px solid #ddd;border-radius:4px'>"
        "<label style='font-weight:600;padding-top:8px'>Start Register:</label>"
        "<input type='number' id='write_multi_start' min='0' max='65535' value='0' style='width:150px;padding:8px;border:1px solid #ddd;border-radius:4px'>"
        "<label style='font-weight:600;padding-top:8px'>Values (comma-separated):</label>"
        "<textarea id='write_multi_values' placeholder='Example: 1000,2000,3000' style='width:100%;max-width:400px;height:70px;padding:8px;border:1px solid #ddd;border-radius:4px;font-family:monospace'></textarea>"
        "</div>"
        "<button onclick='writeMultipleRegisters()' style='background:#28a745;color:white;padding:10px 16px;border:none;border-radius:4px;font-weight:600;cursor:pointer'>Write Multiple Registers</button>"
        "<div id='write_multi_result' style='margin-top:12px;padding:10px;background:#f8f9fa;border-radius:4px;display:none'></div>"
        "</div>"
    );
