...
// ============================================================================
var ws=null;var wsReconnectTimer=null;
...
if(el){el.textContent='LIVE';el.style.color='#10b981';}
console.log('WebSocket connected');
};
ws.onmessage=function(evt){
try{var d=JSON.parse(evt.data);
if(d.type==='sensors'&&d.sensors){
d.sensors.forEach(function(s){
var el=document.getElementById('live-'+s.unit_id);
if(el)el.textContent=s.value.toFixed(2);
var ts=document.getElementById('live-ts-'+s.unit_id);
if(ts)ts.textContent='Updated: '+new Date().toLocaleTimeString();
});
}
if(d.type==='status'){
var h=document.getElementById('ov-heap');if(h)h.textContent=d.heap+' bytes';
var m=document.getElementById('ov-mqtt');if(m){m.textContent=d.mqtt?'Connected':'Disconnected';m.style.color=d.mqtt?'#10b981':'#ef4444';}
var t=document.getElementById('ov-telemetry');if(t)t.textContent=d.telemetry_sent;
}
...
}catch(e){console.log('WS error:',e);wsReconnectTimer=setTimeout(connectWebSocket,5000);}
}
// Auto-connect WebSocket on page load
if(typeof WebSocket!=='undefined'){setTimeout(connectWebSocket,1000);}
