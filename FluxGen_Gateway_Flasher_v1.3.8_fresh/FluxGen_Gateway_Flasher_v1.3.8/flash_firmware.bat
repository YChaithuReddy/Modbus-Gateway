...
if not exist "%~dp0firmware\merged_firmware.bin" (
    ...
    echo.
    echo Run package_firmware.bat first to create the merged firmware.
    echo.
    pause
    exit /b 1
)
...
echo.
echo Installing esptool automatically...
...
echo.
echo Could not install esptool automatically.
echo Please install manually:
echo   pip install esptool
echo.
echo You also need Python installed from https://python.org
echo.
pause
exit /b 1
