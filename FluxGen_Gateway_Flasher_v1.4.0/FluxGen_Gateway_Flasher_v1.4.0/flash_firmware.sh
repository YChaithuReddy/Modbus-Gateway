...
# Auto-package firmware if not already done
if [ ! -f "$SCRIPT_DIR/firmware/merged_firmware.bin" ]; then
    echo "[INFO] Merged firmware not found. Run package_firmware.bat (Windows) first."
    echo "       Or use idf.py build + esptool merge-bin manually."
    exit 1
fi
...
if [ -z "$ESPTOOL_CMD" ]; then
    ...
    if python3 -m esptool version &>/dev/null 2>&1; then
        ...
        echo ""
        echo "[ERROR] Could not install esptool. Please install manually:"
...
read -p "Enter serial port (e.g., /dev/ttyUSB0 or /dev/cu.usbserial-0001): " PORT
...
if [ $? -eq 0 ]; then
    ...
    echo "  - Ping from VPN to wake web server remotely"
else
    echo ""
    echo "========================================"
